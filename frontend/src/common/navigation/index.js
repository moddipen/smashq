...
class Navigation extends React.Component {
  ...
  checkToggle = e => {
    if (this.state.showMenu && !this.popoverRef.current.contains(e.target) && !this.menuRef.current.contains(e.target)) {
      this.setState({
        showMenu: false
      })
    }
  ...
  getPopover = () => {
    ...
    const output = (
      <div className={classes} ref={this.popoverRef}>
        <ul className="menu">
          {pages.map(page => (
            <li><NavLink exact to={page.path} onClick={this.toggleMenu}><i
              className={page.icon}/></NavLink></li>
          ))}
        </ul>
      </div>
    )
  ...
  render () {
    ...
    return (
      <div className={classes}>
        <ul className="menu">
          <li ref={this.menuRef}>
            <a href="javascript:" onClick={this.toggleMenu}><i
              className={this.getSelectedMenuIcon(this.props.selectedMenu)}/></a>
