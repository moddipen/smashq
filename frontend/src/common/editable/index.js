...
class Editable extends React.PureComponent {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      optionsEmpty: typeof(props.optionsEmpty) !== 'undefined' ? props.optionsEmpty : '',
    ...
    if (this.state.showButtons) {
      ...
      return (
        <div className="editable-btn" key={this.props.name + 'editable-btn'}>
          {/*{button}*/}
          <Button color="success" size="sm" onClick={this.onSubmit.bind(this)}
                  key={'btn-success' + this.props.name}>
            <i className="icons-check" key={'icons-check' + this.props.name}/></Button>
          <Button color="danger" size="sm" onClick={this.onCancel.bind(this)}
  ...
  getContent () {
    const {
      ...
      defaultValue, dataType, placement, mode, name, options
    } = this.state
    ...
    if (editable) {
      switch (dataType) {
        ...
        //     break;
        case 'date':
          content.push(<Input type="date" {...componentProps} {...this.state} />)
          break
        // case 'radio':
      ...
      if (mode == 'popup') {
        return (
          <Popover className="inline-edit" isOpen={editable}
                   target={() => ReactDOM.findDOMNode(this.editableAnchor)} toggle={() => {
            this.setEditable(false)
          }} placement={placement}>
            {content}
          </Popover>
    ...
    return (
      <div className={editableContainerClass} key={this.props.name}>
        {!(mode == 'inline' && editable)
          ? (<a ref={ref => this.editableAnchor = ref} onClick={this.setEditable.bind(this, true)}
                href="javascript:" className={this.props.className}>
